...
<section><h2>4. Optional moving-average filter</h2><p>Proximity is {'a required gate' if a.get('monthly_ema_required',True) else 'not a Buy gate'}. The dashboard filter shows fundamentally strong Buy/Watch stocks within ±{proximity}% of a selected average. It changes the displayed list, not the verdict.</p>{li([label for _,label in _MOVING_AVERAGE_ZONES])}<p>SMA gives each sampled closing price equal weight. EMA gives recent prices more weight. Weekly averages use weekly closes, not daily closes. In-progress periods use only prices known so far.</p></section>
