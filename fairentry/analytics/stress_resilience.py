...
def build_stress_resilience(stock_closes: list[float], sector_closes: list[float],
    ...
    return {
        ...
        "candidate_rule": "Strong: the stock typically fell no more than its benchmark, at least 60% of past cases recovered within one year, and the typical recovery took no more than 180 trading days. Acceptable: the stock typically lagged its benchmark by no more than 5 percentage points and at least half of past cases recovered.",
        "backtestable": True,
        "validated_for_score": False,
        "score_effect": 0,
        "verdict_effect": "none",
        "observed_at": observed_at,
        "policy": "Research only. Past recoveries do not predict the next decline and do not change today's score or recommendation.",
        "data_note": "The live calculation lines up the most recent stock and benchmark trading days. Before this could affect a recommendation, the historical test must match every value by its exact date.",
    }
